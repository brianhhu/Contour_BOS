function [seed, stim] = gen_multcontstim( Ni, Ng, num_ori, num_pos, b_len, c_cen, c_ori, c_len, saved_seed )
...
if size(c_cen,1) < 2
    error('Not enough center points');
end

if length(c_len) < 2
    error('Not enough contour lengths');
end

if length(c_ori) < 2
    error('Not enough contour orientations');
end
...
Ng_ind_x = floor((Ni(2)-(Ng(2)-1)*floor(Ni(2)/Ng(2)))/2):floor(Ni(2)/Ng(2)):(Ni(2)-ceil(floor(Ni(2)/Ng(2))/2));
Ng_ind_y = floor((Ni(1)-(Ng(1)-1)*floor(Ni(1)/Ng(1)))/2):floor(Ni(1)/Ng(1)):(Ni(1)-ceil(floor(Ni(1)/Ng(1))/2));
...
for i = 1:num_ori
    ...
    if (ori == 45 || ori == 135) && b_len > 3
        ori_se{i} = strel('line', b_len+2, ori);
    else
...
cont_ind_x = fliplr((c_cen(1,2)-x_mult(1)*(c_len(1)-ceil(c_len(1)/2))):x_mult(1):(c_cen(1,2)+x_mult(1)*(c_len(1)-ceil(c_len(1)/2)))); % centered
cont_ind_x2 = fliplr((c_cen(2,2)-x_mult(2)*(c_len(2)-ceil(c_len(2)/2))):x_mult(2):(c_cen(2,2)+x_mult(2)*(c_len(2)-ceil(c_len(2)/2)))); % centered

cont_ind_y = (c_cen(1,1)-y_mult(1)*(c_len(1)-ceil(c_len(1)/2))):y_mult(1):(c_cen(1,1)+y_mult(1)*(c_len(1)-ceil(c_len(1)/2)));
cont_ind_y2 = (c_cen(2,1)-y_mult(2)*(c_len(2)-ceil(c_len(2)/2))):y_mult(2):(c_cen(2,1)+y_mult(2)*(c_len(2)-ceil(c_len(2)/2))); % two contours

% repmat for empty matrices - repeat along midline
if isempty(cont_ind_x)
    cont_ind_x = repmat(c_cen(1,2), 1, c_len(1));
end
if isempty(cont_ind_x2)
    cont_ind_x2 = repmat(c_cen(2,2), 1, c_len(2));
end
if isempty(cont_ind_y)
    cont_ind_y = repmat(c_cen(1,1), 1, c_len(1));
end
if isempty(cont_ind_y2)
    cont_ind_y2 = repmat(c_cen(2,1), 1, c_len(2));
end

% Find linear indices of contour within image and replace with new
% orientation/position
cont_lin_ind = sub2ind([Ng(1) Ng(2)], cont_ind_y, cont_ind_x);
cont_lin_ind2 = sub2ind([Ng(1) Ng(2)], cont_ind_y2, cont_ind_x2);

G_ori(cont_lin_ind) = c_ori(1);
G_pos(cont_lin_ind) = 1;

G_ori(cont_lin_ind2) = c_ori(2);
G_pos(cont_lin_ind2) = 1;
...
% % to test position dependence
% shift = 3; % how much to translate contour
% stim_pad = padarray(stim(:,1:end-1,1:end-1), [0 shift shift], 'circular');
% stim_pad_trans = imtranslate(stim_pad, [shift 0 0]);
% stim = stim_pad_trans(:,shift+1:end-shift+1,shift+1:end-shift+1);
